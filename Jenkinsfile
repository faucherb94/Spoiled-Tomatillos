		...
		stage('Deploy') {
			steps {
				...
				withCredentials ([file(credentialsId: 'CS4500_AWS_PEM_File', variable: 'PEM_PATH')]) {
					...
					sh 'ssh -o StrictHostKeyChecking=no -i $PEM_PATH ec2-user@app.codersunltd.me export DB_PASS=$DB_PASSWORD && nohup java -jar app/cs4500-spring2018-team26-1.war > app.out 2>&1 &'
				}
			}
		}
	}
